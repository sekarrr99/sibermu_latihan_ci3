...
class Admin extends CI_Controller {
    ...
    public function index()
	{
		...
		$data['akun']= $this->db->get_where('user')->result_array();
		$data['title']= 'Admin Sekolah X';
	...
	{
		$data['user']= $this->db->get_where('user',['email'=> $this->session->userdata('email')])->row_array();
		$data['student']= $this->db->get_where('siswa')->result_array();
		$data['title']= 'Admin Sekolah X';
		$this->load->view('templates/menuheader', $data);
		$this->load->view('templates/menusidebar', $data);
	...
	public function delete($nis){
		$where= array('nis' => $nis);
		$this->m_user->delete_data($where, 'siswa');
	...
	public function update_student(){
		...
		$where=array('nis'=>$nis);
		$this->m_user->edit($where, $data, 'siswa');
		redirect('admin/datasiswa');
	}
}